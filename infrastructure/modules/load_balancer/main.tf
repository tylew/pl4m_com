...
resource "google_compute_backend_bucket" "pdf_backend" {
  name        = "pdf-backend"
...
resource "google_compute_managed_ssl_certificate" "lb_certificate_pdf_archive" {
  name = "lb-certificate-pdf-archive"

  managed {
    domains = ["pdf.${var.domain}"]
  }
}
...
resource "google_compute_url_map" "lb_url_map" {
  ...
  host_rule {
    hosts        = ["pdf.${var.domain}"]
    path_matcher = "pdf-paths"
  }

  path_matcher {
    name            = "pdf-paths"
    default_service = google_compute_backend_bucket.pdf_backend.self_link
  }
...
resource "google_compute_target_https_proxy" "lb_https_proxy" {
  ...
  ssl_certificates = [
    google_compute_managed_ssl_certificate.lb_certificate.id,  # Old cert
    google_compute_managed_ssl_certificate.lb_certificate_pdf_archive.id  # New cert
  ]
}
